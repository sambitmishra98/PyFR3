...
class CUDAGiMMiKKernels(CUDAKernelProvider):
    ...
    def mul(self, a, b, out, alpha=1.0, beta=0.0):
        ...
        nkerns = self.cfg.getint('backend-opencl', f'gimmik_mm_{a.nrow}x{a.ncol}', self.nkerns)
            ...
            try:
                for i in range(nkerns):
                    ...
                    self.backend.bench_kern |= {
                                f'test-{i}-{kname}_a-rows': a.nrow, 
                                f'test-{i}-{kname}_a-cols': a.ncol,
                                f'test-{i}-{kname}_b-rows': b.nrow, 
                                f'test-{i}-{kname}_b-cols': b.ncol,
                                f'test-{i}-{kname}_out-rows': out.nrow, 
                                f'test-{i}-{kname}_out-cols': out.ncol,
                                f'test-{i}-{kname}_nz': a.nrow*a.ncol-np.count_nonzero(arr),
                                f'test-{i}-{kname}_nnz': np.count_nonzero(arr),
                                f'test-{i}-{kname}_gs-grid': meta['grid'], 
                                f'test-{i}-{kname}_ls-block-tgrp': meta['block'],
                                f'test-{i}-{kname}_Runtime': dt}

                    if best_kern is None or dt < best_kern[-1]:
                        best_kern = kern, meta['grid'], meta['block'], dt

                        self.backend.bench_kern |= {f'best-choice_{kname}': i}
