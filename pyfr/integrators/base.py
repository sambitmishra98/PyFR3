...
#        sim_rep_stats = self.collect_device_on_node(sim_rep_stats)
    ...
    #        2*row['nnz']*row['b.ncol']/row['Runtime']/1e12            
            ...
            if 'Runtime' in k:
                ...
                nnz = self.backend.bench_kern[f'{kname}_nnz']
                b = self.backend.bench_kern[f'{kname}_b-cols']

                performance = 2*nnz*b/v
                 
                benchmark_stats.set('benchmark', f'{knumber}_{kname}', performance)
                benchmark_stats.set('benchmark', f'{knumber}_gimmik_{self.backend.bench_kern[f"{kname}_a-tags"]}', performance)
