...
class OptimisationStatsPlugin(BaseSolnPlugin):
    ...
    def __init__(self, intg, cfgsect, suffix):
        ...
        self.dtau_init = self.cfg.getfloat(tsect, 'pseudo-dt')
        self.dtau_controller = self.cfg.get(tsect, 'pseudo-controller')
    ...
    def collect_stats(self, intg):
        ...
        dc = intg._compute_time - self.ctime_p  
        intg.dc = dc
        # Wall-time per physical time-step
        dw = (time() - intg._wstart) - self.wtime_p
        self.dcdt = dc/dt

        self.wtime_p = time() - intg._wstart                # prev   wall  time
        self.ctime_p = intg._compute_time  # prev compute time

        t1 = pd.DataFrame({ 'physical-time': [intg.tcurr - intg.dt], 
                            'compute-dt'   : [dc], 
                               'wall-dt'   : [dw],
                            'cost'         : [self.dcdt],
                          }) 
    ...
    def check_status(self, intg):
        ...
        # Typical case: If dtau is constant and enough data is collected
        if ((self.dtau_controller == 'none'
              or (self.dtau_controller == 'local-pi'
                  and abs(intg.pseudointegrator.pintg.dtau_max
                          - self.pd_stats['max-dtau'][self.pd_stats.index[-1]]
                         )<1e-6
                  )
             )
