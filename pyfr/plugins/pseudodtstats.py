...
class PseudodtStatsPlugin(BaseSolnPlugin):
    ...
    def __call__(self, intg):
        ...
        for (npiter, iternr, resid) in intg.pseudostepinfo:
            ...
            if iternr == 1:           # We can store the last step's data
                if 'solver-dual-time-integrator-multip' in intg.cfg.sections():
                    deltau_mats = intg.pseudointegrator.pintgs[self.level].dtau_mats
                else:
                    deltau_mats = intg.pseudointegrator.dtau_mats
            ...
            self.deltau_statistics(intg, deltau_mats)
            #self.residual_statistics(intg, resid)
            self.last_appendable = (npiter, intg.tcurr, 
                                    *self.deltau_stats_as_list(intg.dtau_stats))
    ...
    def deltau_statistics(self, intg, deltau_mats):
        '''
        ...
        for j, var in enumerate(self.fvars):
            for i, e_type in enumerate(self.e_types):
                ...
                intg.dtau_stats['min'][var][e_type]['each'] = (
                                                    deltau_mats[i][:, j, :].min(1))
                intg.dtau_stats['max'][var][e_type]['each'] = (
                                                    deltau_mats[i][:, j, :].max(1))
            ...
            t_max = np.array(intg.dtau_stats['min'][var]['all'])
            ...
            intg.dtau_stats['min'][var][e_type]['all'] = t_max

            t_min = np.array(intg.dtau_stats['max'][var]['all'])
            ...
            intg.dtau_stats['max'][var][e_type]['all'] = t_min

        # Stats obtained over all element types, elements, variable in (p, u, v, w) and element soln points
        intg.dtau_stats['min']['all'] = min([intg.dtau_stats['min'][var]['all'] for var in self.fvars])
        intg.dtau_stats['max']['all'] = max([intg.dtau_stats['max'][var]['all'] for var in self.fvars])
    ...
    def residual_statistics(self, intg, resid):
        ...
        if self.cfg.get('solver-time-integrator','pseudo-resid-norm')=='uniform':
            intg.dtau_stats['res']['all'] = max([intg.dtau_stats['res'][var]['all'] for var in self.fvars])
        elif self.cfg.get('solver-time-integrator', 'pseudo-resid-norm')=='l2':
            intg.dtau_stats['res']['all'] = sum([intg.dtau_stats['res'][var]['all'] for var in self.fvars])
        else:
    ...
    def residual_statistics_next_gen(self, intg, resid):
        ...
        for j, var in enumerate(self.fvars):
            ...
            # each variable in (p, u, v, w)
            # Stats obtained over all element types, elements and element soln points
            intg.dtau_stats['res'][var]['all'] = max([intg.dtau_stats['res'][var][e_type]['all'] for e_type in self.e_types])
