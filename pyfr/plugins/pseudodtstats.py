...
class PseudoDtStatsPlugin(BaseSolnPlugin):
    ...
    def __init__(self, intg, cfgsect, prefix):
        ...
        if self.abstraction > 3:
            raise ValueError('abstraction > 3 has not been implemented yet')
    ...
    def __call__(self, intg):
        ...
        for (npiter, iternr, resid) in intg.pseudostepinfo:
            ...
            if iternr == 1:           # We can store the last step's data
                if 'solver-dual-time-integrator-multip' in intg.cfg.sections():
                    dtau_mats = intg.pseudointegrator.pintgs[self.level].dtau_mats
                else:
                    dtau_mats = intg.pseudointegrator.dtau_mats
            ...
            self.dtau_statistics(intg, dtau_mats)
            self.residual_statistics(intg, resid)
            self.last_appendable = (npiter, intg.tcurr, 
                                    *self.dtau_stats_as_list(intg.dtau_stats))
    ...
    def dtau_statistics(self, intg, dtau_mats):
        '''
            self.dtau_mats is a list of matrices, one for each element type.
    ...
    def dtau_stats_as_list(self, dtau_stats):
        dtau_stats_list = []
        for v in dtau_stats.values():
            dtau_stats_list.append(v['all'])               
            for vk, vv in v.items():
                if self.abstraction>1:                   
                    if isinstance(vv, dict):    
                        dtau_stats_list.append(vv['all'])
                if vk != 'all':
                    for vvv in vv.values():
                        if self.abstraction>2:
                            if isinstance(vvv, dict):
                                dtau_stats_list.append(vvv['all'])

        return dtau_stats_list