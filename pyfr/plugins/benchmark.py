...
class BenchmarkCLI(BaseCLIPlugin):
    ...
    def inifile_as_dict(self, config_file):
        ...
        for section in config_file.sections():
            for option in config_file.items(section):
                stats_dict[f'{section}-{option}'] = config_file.get(section, option)
        return stats_dict
    ...
    def pyfrs_to_csv(self, files):
        ...
        for file in files:
            with h5py.File(file, 'r') as f:
                ...
                # Save all stats entries in the dataframe, with column name as {section}-{option}
                # Create a dictionary from stats, then append to the dataframe
                stats_dict = {}
                stats_dict['file-name'] = file

                # Combine
                df_dict = {**self.inifile_as_dict(config),
                                **self.inifile_as_dict(stats)}
    ...
    def process_columns(self, df, remove_if_present=[], include_if_present=[]):
        for column in df.columns:
            if column in remove_if_present:
                df.drop(column, axis=1, inplace=True)
            if column not in include_if_present:
                df.drop(column, axis=1, inplace=True)
        return df
